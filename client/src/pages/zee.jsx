...
//         // Base query for PV expenses (Paid status only)
//         let expenseQuery = `
//           SELECT
...
//             COALESCE(AVG(pv.total_amount), 0) as average_expense
//           FROM pv_headers pv
...
//         let queryParams = [defaultStartDate, defaultEndDate];

//         // Add category filter if provided
//         if (category) {
//           expenseQuery += ` AND pi.expense_category = ?`;
//           queryParams.push(category);
//         }

//         // Add payee filter if provided
//         if (payee) {
//           expenseQuery += ` AND pv.paid_to LIKE ?`;
//           queryParams.push(`%${payee}%`);
//         }

//         const [expenses] = await pool.query(expenseQuery, queryParams);

//         // Get expense details if requested
//         let expenseDetails = [];
//         if (include_details === "true") {
//           let detailsQuery = `
//             SELECT
...
//           let detailsParams = [defaultStartDate, defaultEndDate];

//           if (category) {
//             detailsQuery += ` AND pi.expense_category = ?`;
//             detailsParams.push(category);
//           }

//           if (payee) {
//             detailsQuery += ` AND pv.paid_to LIKE ?`;
//             detailsParams.push(`%${payee}%`);
//           }

//           detailsQuery += ` ORDER BY pv.pv_date DESC`;

//           const [details] = await pool.query(detailsQuery, detailsParams);
//           expenseDetails = details;
//         }

//         // Calculate total from the query result
//         const totalExpenses = parseFloat(expenses[0]?.total_expenses || 0);
//         const totalPVs = parseInt(expenses[0]?.total_pvs || 0);
//         const uniqueCategories = parseInt(expenses[0]?.expense_categories || 0);
//         const uniquePayees = parseInt(expenses[0]?.unique_payees || 0);
//         const averageExpense = parseFloat(expenses[0]?.average_expense || 0);

//         const response = {
//           expenditure: {
//             total: totalExpenses,
//             total_pvs: totalPVs,
//             average_per_pv: averageExpense,
...
//         if (include_details === "true") {
//           response.expense_details = expenseDetails;

//           // Add summary by category
//           const categorySummary = {};
//           expenseDetails.forEach((detail) => {
//             if (!categorySummary[detail.expense_category]) {
//               categorySummary[detail.expense_category] = {
//                 category: detail.expense_category,
//                 total_amount: 0,
//                 item_count: 0,
//                 pv_count: new Set(),
//               };
//             }
//             categorySummary[detail.expense_category].total_amount += parseFloat(
//               detail.amount || 0,
//             );
//             categorySummary[detail.expense_category].item_count++;
//             categorySummary[detail.expense_category].pv_count.add(
//               detail.pv_number,
//             );
//           });

//           response.category_summary = Object.values(categorySummary)
//             .map((cat) => ({
//               ...cat,
//               pv_count: cat.pv_count.size,
//             }))
...
//         res.status(500).json({
//           error: "Failed to fetch expenditure report",
